...
const os = require('os');
let { WORKTYPE, PREFIX, BOT_INFO } = require("../config");
let img_url,
  theme = "text";
...
async function send_menu(m) {
  ...
  let category = [],
    menu = `╭━〔 ${(info_vars[0] || info_vars || "").replace(/[;,|]/g, "")} 〕━◉
┃╭━━━━━━━━━━━━━━◉
...
┃╰━━━━━━━━━━━━━◉`;
  commands.forEach((command) => {
    if (
      ...
      command.pattern &&
      !category.includes(command.type.toLowerCase())
    ) {
      category.push(command.type.toLowerCase());
    }
  });
  category.forEach((cmmd) => {
    menu += `
┠┌─⭓『 ${addSpace("*" + cmmd.toUpperCase() + "*", 14, "both")} 』`;
    let comad = commands.filter(({ type }) => type.toLowerCase() === cmmd);
    comad.forEach(({ pattern }) => {
      if (pattern) {
        menu += `\n┃│◦ _${pattern.replace(/[^a-zA-Z0-9-+]/g, "")}_`;
      }
    });
    menu += `\n┃└──────────⭓`;
  });
