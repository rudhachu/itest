...
╰───────────────────···•\n${readMore}`;
        ...
        commands.forEach((command) => {
            ...
            } else if (typeof command.pattern === "string") {
                cmd = command.pattern;
            }

            if (!command.dontAddCommandList && cmd) {
                let type = command.type ? command.type.toLowerCase() : "misc";
                commandList.push({ cmd, type });
                categories.add(type);
            }
        });
        ...
        [...categories].sort().forEach((category) => {
            ...
            let filteredCommands = commandList.filter(({ type }) => type === category);
        ...
        // Handling link preview if enabled
        if (config.LINK_PREVIEW && !["null", "false"].includes(config.LINK_PREVIEW.toLowerCase())) {
        ...
        if (theme === "text") {
            return await m.client.sendMessage(m.jid, { text: menu, ...options });
        } else {
