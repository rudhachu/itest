...
async function send_menu(m) {
    ...
    const hrs = new Intl.DateTimeFormat("en-IN", { ...options, hour: "2-digit" }).format(now);
    const [date, time] = new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", dateStyle: "full", timeStyle: "short" }).format(now).split(" at ");

    const type = mode ? "Private" : "Public";
    let wish = "";
    if (hrs < 12) wish = "⛅ Good Morning";
    else if (hrs >= 12 && hrs <= 16) wish = "🌞 Good Afternoon";
    else if (hrs >= 16 && hrs <= 20) wish = "🔆 Good Evening";
    else wish = "🌙 Good Night";

    const readMore = "\u200B".repeat(4001); // Unicode zero-width space
...
╰───────────────────•\n ${readMore}`;
    ...
    commands.forEach((command) => {
        if (command.pattern instanceof RegExp) {
            let cmd = String(command.pattern).split(/\W+/)[1];
    ...
    let mediaUrl = config.BOT_INFO.split(";")[2];

    return await message.sendFromUrl(mediaUrl, { caption: menu, quoted: message });
}
