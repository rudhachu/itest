...
async function send_menu(m) {
	try {
		let img_url = null;
		let theam = "text"; // Default to text

		// Check if BOT_INFO has valid media
...
│╭───────────────···▸
┴│      Hey 👋  ${m.pushName}
⬡│▸   *Bot Name* : ${config.BOT_INFO.split(";")[0] || "Unknown"}
⬡│▸   *Version*  : ${require("../package.json").version}
⬡│▸   *Prefix*  : ${PREFIX}
⬡│▸   *Mode*  : ${config.WORKTYPE}
⬡│▸   *Commands*  : ${commands.length.toString()}
⬡│▸   *Date*  : ${date}
⬡│▸   *RAM Usage*  : ${format(os.totalmem() - os.freemem())}
⬡│            
⬡│           █║▌║▌║║▌║ █
┬│            ʀ   ᴜ   ᴅ   ʜ   ʀ   ᴀ
│╰────────────────···▸
╰───────────────────···•\n ${readMore}`;
		...
		commands.forEach((command) => {
			if (command.pattern) {
				const type = command.type?.toLowerCase() || "misc";
		...
		// Format command list
		Object.keys(types).forEach((cmmd) => {
			menu += `\n┌───〈 *${cmmd.toUpperCase()}* 〉───◆`;
			menu += `\n│╭─────────────···▸`;
			menu += `\n┴│▸`;
			types[cmmd].forEach((cmd) => {
				menu += `\n⬡│▸  ${cmd.trim()}`;
			});
			menu += `\n┬│▸`;
			menu += `\n│╰────────────···▸▸`;
		...
		// Check if LINK_PREVIEW is enabled
		if (
			config.LINK_PREVIEW &&
			typeof config.LINK_PREVIEW === "string" &&
			config.LINK_PREVIEW.toLowerCase() !== "null" &&
			config.LINK_PREVIEW.toLowerCase() !== "false"
		) {
			options.contextInfo.externalAdReply = {};
			const linkImage = MediaUrls(config.LINK_PREVIEW);

			if (linkImage && Array.isArray(linkImage) && linkImage.length > 0) {
				config.LINK_PREVIEW = config.LINK_PREVIEW.replace(linkImage[0], "");
				options.contextInfo.externalAdReply.thumbnailUrl = linkImage[0];
			}

			options.contextInfo.externalAdReply.title =
				config.LINK_PREVIEW.split(/[;,|]/)[0] || "Check this out!";
			options.contextInfo.externalAdReply.body =
				config.LINK_PREVIEW.split(/[;,|]/)[1] || "";
			options.contextInfo.externalAdReply.sourceUrl =
				extractUrlsFromString(config.LINK_PREVIEW)?.[0] || "";
		}

		// Send message
		if (img_url) {
			// Send image or video if available
		...
		console.error("Error in send_menu:", error);
	}
}
