...
class serialize {
    ...
    if (/webp/.test(mimetype)) {
        ...
        if (typeof file === "string") {
            file = fs.readFileSync(file); // Read the file into a buffer
        }

        // Convert audio format
        let converted = await (ptt ? toPTT : toAudio)(file, type.ext);
        let detected = await fileType.fromBuffer(converted.data);
        
        if (!detected) throw new Error("Unable to detect file type");

        let tempFilePath = generateFileName(detected.ext);
        fs.writeFileSync(tempFilePath, converted.data);
