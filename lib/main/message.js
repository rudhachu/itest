...
async sendFromUrl(path, filename = "", caption = "", quoted, ptt = true, options = {}) {
    ...
    if (!type && options.asDocument) options.asDocument = true;

    let mtype = "", mimetype = type.mime;
    let generateFileName = (ext) => `../../media/${Date.now()}.${ext}`;
    ...
    if (/webp/.test(mimetype)) {
        ...
        let converted = await (ptt ? toPTT : toAudio)(file, type.ext);
        let detected = await require("file-type").fromBuffer(converted.data);
        let tempFilePath = generateFileName(detected.ext);

        require("fs").writeFileSync(tempFilePath, converted.data);
        pathFile = tempFilePath;
        mtype = "audio";
        mimetype = "audio/mpeg";
    } else {
        mtype = "document";
    ...
    .then(() => {
        ...
        require("fs").unlinkSync(pathFile);
    });
};
