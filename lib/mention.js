async function mention(m, text) {
	...
	if (type !== 'text' && URLS[0]) {
		...
		if (type == 'image') {
			...
			// Fix: check if audio is a voice note (in .opus format)
			message.mimetype = URL.endsWith('.opus') ? 'audio/ogg; codecs=opus' : 'audio/mpeg';
			message.audio = { url: URL };

			// If it's a voice note, set ptt (push-to-talk)
